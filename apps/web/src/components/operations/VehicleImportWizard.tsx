...
interface ImportResponse {
  ...
  errors: number;
  importLogId: string;
  vehicles: Array<{ id: string; code: string; name: string; licensePlate: string }>;
}
...
export function VehicleImportWizard({ onClose, onImported }: Props) {
  ...
  /* Picking a new file resets every downstream state — the prior preview/result
     no longer matches the file the user is about to send. */
  ...
  const handleDownloadTemplate = async () => {
    try {
      const blob = await apiClient.fetchBlob('/api/operations/fleet/vehicles/import/template');
  ...
  const goToPreview = async () => {
    ...
    try {
      ...
      const res = await apiClient.uploadFile<PreviewResponse>(
        '/api/operations/fleet/vehicles/import/preview',
        fd,
      );
      setPreview(res);
      setStep('preview');
  ...
  const doImport = async () => {
    ...
    try {
      ...
      const res = await apiClient.uploadFile<ImportResponse>(
        '/api/operations/fleet/vehicles/import',
        fd,
      );
      setResult(res);
      setStep('result');
  ...
  return (
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
      <div className="bg-card-solid rounded-xl shadow-xl w-full max-w-3xl max-h-[92vh] overflow-y-auto">
        ...
        {/* Footer */}
        <div className="flex items-center justify-between gap-2 px-5 py-3 border-t border-[var(--border-color)] sticky bottom-0 bg-card-solid">
          {step === 'upload' && (
            <>
              <button
                onClick={onClose}
          ...
          {step === 'preview' && preview && (
            <>
              <button
                ...
                className="inline-flex items-center gap-1.5 px-4 py-2 text-sm border border-line text-fg rounded-lg hover:bg-subtle-hover"
                style={{ fontFamily: 'var(--font-outfit), sans-serif', fontWeight: 500 }}
              >
              ...
              >
                {importing
                  ...
                  : `Importar ${preview.validRows} ${
                      preview.validRows === 1 ? 'vehículo válido' : 'vehículos válidos'
                    }`}
              </button>
            </>
          )}
  ...
  return (
    <div className="space-y-4">
      <p className="text-sm text-[var(--text-secondary)]" style={{ lineHeight: 1.5 }}>
        Carga masivamente vehículos desde un archivo CSV o Excel. El tipo de activo debe ser de
        categoría <strong>VEHÍCULO</strong>; los nombres de tipos, subtipos y ubicaciones se
        reconocen literalmente, así que configúralos antes de importar.
      </p>
      ...
      >
        {file ? (
          <div className="flex items-center justify-center gap-3">
            <FileSpreadsheet size={28} style={{ color: 'var(--color-accent)' }} />
            <div>
              <div
                style={{
                  fontFamily: 'var(--font-outfit), sans-serif',
...
function PreviewStep({
  ...
  /* Cap rendering at 200 rows to keep the modal responsive — totals reflect
     the full preview. */
    ...
    <div className="space-y-4">
      ...
      <div className="space-y-2">
        ...
        <ToggleRow
          ...
          description="Los nuevos vehículos quedan en estado FUERA_DE_SERVICIO en vez de OPERATIONAL. Útil para revisar antes de activar."
        />
      </div>
          ...
          <tbody>
            {displayRows.map((row) => (
              <tr key={row.rowNumber}>
                ...
                <td style={{ fontFamily: 'var(--font-outfit), sans-serif' }}>
                  {row.data.name || '—'}
                </td>
    ...
    <div className="space-y-4">
      ...
      >
        ...
        <p className="text-sm text-[var(--text-secondary)]">
          {result.imported} {result.imported === 1 ? 'vehículo creado' : 'vehículos creados'}
            ...
            <tbody>
              {result.vehicles.map((v) => (
                <tr key={v.id}>
                  ...
                  <td style={{ fontFamily: 'var(--font-outfit), sans-serif' }}>{v.name}</td>
                </tr>
              ))}
            </tbody>
          </table>
