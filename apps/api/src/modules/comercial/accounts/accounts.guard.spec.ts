/* COM-003 — proves the Account authorization matrix the controller relies on:
 * READ reaches MANAGER/ADMIN/SUPER_ADMIN + ACCOUNTANT (read-only portfolio
 * visibility); WRITE (create/update/delete) is MANAGER/ADMIN/SUPER_ADMIN only.
 * ANALYST/VIEWER stay on the COM-001 default-deny floor (no read, no write).
 * ACCOUNTANT write === false is pinned explicitly (read-only guard). */
import { UserRole } from '@prisma/client';
import { AccountSubject, CaslAbilityFactory } from '../../common/casl/casl-ability.factory';

describe('COM-003 Account authorization matrix', () => {
  const factory = new CaslAbilityFactory();
  const can = (role: UserRole, action: 'read' | 'create' | 'update' | 'delete') =>
    factory.defineAbilityFor(role).can(action, AccountSubject);
