...
export class PermitsService {
  ...
  async approve(id: string, companyId: string, userId: string) {
    ...
    const final = await this.findOne(id, companyId);
    /* OPS-033 — when the chain completes the permit flips to
       APPROVED. That's the moment we sweep any pending
       auto-commitment for an earlier version of the same
       (target, permitType). We match permitType by code via
       sourceMetadata since renewal events stash that string. */
    if (final.status === 'APPROVED') {
      try {
        await this.autoFulfillRenewalCommitment(companyId, {
          permitTypeCode: final.permitType.code,
          assetId: final.assetId,
          locationId: final.locationId,
          triggeringPermitId: id,
        });
      } catch (err) {
        this.logger.warn(
          `Auto-fulfill commitment after permit approve failed (${id}): ${
  ...
  /* OPS-033 — clear any open auto-commitment whose source was an
     earlier version of this (target, permitType). Mirror of the
     document-side helper: keyed off sourceMetadata.permitTypeCode +
     either assetId or locationId so we don't auto-fulfill across
     unrelated targets that share a permitType. */
  private async autoFulfillRenewalCommitment(
    companyId: string,
    args: {
      permitTypeCode: string;
      assetId: string | null;
      locationId: string | null;
      triggeringPermitId: string;
    },
  ): Promise<void> {
    const targetClause: Prisma.CommitmentWhereInput = args.assetId
      ? { sourceMetadata: { path: ['assetId'], equals: args.assetId } }
      : args.locationId
        ? { sourceMetadata: { path: ['locationId'], equals: args.locationId } }
        : /* No target — skip rather than match every permit. */
          { id: '__never_matches__' };
    const candidate = await this.prisma.commitment.findFirst({
      where: {
        companyId,
        sourceType: 'permit_renewal',
        isAutoGenerated: true,
        status: 'PENDING',
        autoFulfilledAt: null,
        AND: [
          { sourceMetadata: { path: ['permitTypeCode'], equals: args.permitTypeCode } },
          targetClause,
        ],
      },
    ...
    await this.prisma.commitment.update({
      ...
      data: {
        status: 'FULFILLED',
        autoFulfilledAt: new Date(),
        autoFulfilledBy: `permit_renewal:${args.triggeringPermitId}`,
      },
    });
    this.logger.log(
      `Auto-fulfilled commitment ${candidate.id} via permit approval ${args.triggeringPermitId}`,
    );
  }
