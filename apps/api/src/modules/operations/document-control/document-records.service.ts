...
export class DocumentRecordsService {
  ...
  async approve(id: string, companyId: string, userId: string) {
    ...
    /* OPS-033 — auto-fulfill any pending auto-generated commitment
       that was created for an earlier version of this same
       (asset, documentType). We match via the JSONB metadata
       written by the listener since we don't keep a direct FK
       from Commitment to DocumentRecord. The match is fuzzy on
       purpose: a renewal is "any newly-approved doc of the same
       type for the same asset", regardless of supersession links. */
    try {
      await this.autoFulfillRenewalCommitment(companyId, {
        assetId: existing.assetId,
        documentTypeId: existing.documentTypeId,
        triggeringRecordId: id,
      });
    } catch (err) {
      this.logger.warn(
        `Auto-fulfill commitment after approve failed (doc ${id}): ${
          err instanceof Error ? err.message : err
        }`,
      );
    }
  ...
  /* OPS-033 — clear any open auto-commitment whose source was an
     earlier version of this (asset, documentType). Marks PENDING
     rows as FULFILLED with autoFulfilledAt/By so cashflow
     dashboards can distinguish them from real cash-paid rows. We
     update at most one row per call (the oldest PENDING) — if
     there are stragglers a future renewal will sweep them. */
  private async autoFulfillRenewalCommitment(
    companyId: string,
    args: { assetId: string; documentTypeId: string; triggeringRecordId: string },
  ): Promise<void> {
    const candidate = await this.prisma.commitment.findFirst({
      where: {
        companyId,
        sourceType: 'document_renewal',
        isAutoGenerated: true,
        status: 'PENDING',
        autoFulfilledAt: null,
        AND: [
          { sourceMetadata: { path: ['assetId'], equals: args.assetId } },
          { sourceMetadata: { path: ['documentTypeId'], equals: args.documentTypeId } },
        ],
      },
    ...
    await this.prisma.commitment.update({
      ...
      data: {
        ...
        autoFulfilledBy: `document_renewal:${args.triggeringRecordId}`,
      },
    });
    this.logger.log(
      `Auto-fulfilled commitment ${candidate.id} via document approval ${args.triggeringRecordId}`,
    );
  }
