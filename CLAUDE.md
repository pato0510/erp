...
The frontend uses a custom design system called "Terminal Noir".

### Tokens

- --color-dark: #0C0F1A (backgrounds oscuros, sidebar, botón primario)
- --color-accent: #4ECDC4 (teal — acentos, links, checkmarks, cursor)
- --color-accent-dim: rgba(78, 205, 196, 0.15) (fondos hover teal)
- --color-surface: #fafafa (inputs, cards)
- --color-border: #e8eaed (bordes claros)
- --color-border-dark: #1a2535 (bordes en panel oscuro)
- --color-text-muted: #9aa0ad (textos secundarios)

### Fonts

- --font-mono: 'JetBrains Mono' — labels, datos, sidebar nav, terminal blocks
- --font-display: 'Syne' — títulos y headlines
- --font-body: 'Inter' — texto general, formularios

### Layout pattern

- Panels oscuros (#0C0F1A): JetBrains Mono para todo el texto
- Panels claros (white): Inter para cuerpo, Syne para títulos
- Acento único: #4ECDC4 — no usar otros colores de acento

### Components

- Sidebar: fondo #0C0F1A, nav en JetBrains Mono 12px,
  item activo con border-left teal + texto teal
- Botón primario: fondo #0C0F1A, texto blanco Syne,
  con círculo teal y flecha →
- Inputs: border #e8eaed, border-radius 8px,
  focus border #4ECDC4, background #fafafa
- Terminal block: fondo #080b14, border #1a2535,
  texto JetBrains Mono, checkmarks y prefijos en teal
- Cards: border #e8eaed 1px, border-radius 12px, fondo blanco

### Rules

- NUNCA usar otros colores de acento (no purple, no blue, no orange)
- NUNCA usar Inter en paneles oscuros — siempre JetBrains Mono
- NUNCA usar gradientes
- Íconos: lucide-react, tamaño 16px en nav, 20px en contenido
- Todos los montos monetarios con formatCLP() de lib/formatters.ts
